	...
	const bool include_semantics)
{
  ...
  if (include_semantics) {
		out_semantics = torch::full({NUM_CHANNELS_semantics, H, W}, 0.0, float_opts);
	  }
  else {
		out_semantics = torch::full({1}, 0.0, float_opts);
	  }
	...
	const bool include_semantics) 
{
  ...
  if (include_semantics) {
		dL_dsemantics = torch::zeros({P, NUM_CHANNELS_semantics}, means3D.options());
	} else {
		dL_dsemantics = torch::zeros({1}, means3D.options());
	}
